...
def enhance_one_track(
    model, audio_path, saved_dir, cut_len, n_fft=400, hop=100, save_tracks=False, db_increase=0
):
    name = os.path.split(audio_path)[-1]
    noisy, sr = torchaudio.load(audio_path)

    amplitude_factor = 10 ** (db_increase / 20)
    assert sr == 16000
    noisy = noisy.cuda()
    ...
    est_audio = est_audio * amplitude_factor
