...
parser.add_argument("--loss_weights", type=list, default=[0.1, 0.7, 0.15, 0.05],
                    help="weights of RI components, magnitude, time loss, and Metric Disc") #default=[0.1, 0.9, 0.2, 0.05],
args = parser.parse_args()
logging.basicConfig(level=logging.INFO)


def eval_epochs(G_losses, model_path, min_epochs):
    best_epoch = np.argmin(np.absolute(G_losses[round(min_epochs):]))
    best_epoch = best_epoch + round(min_epochs)

    for filename in os.listdir(model_path):
        if f'_{best_epoch}.pth' not in filename:
...
class Trainer:
    ...
    def calculate_generator_loss(self, generator_outputs):
        ...
        x = generator_outputs["one_labels"].float() # added [0]
        gen_loss_GAN = F.mse_loss(
            predict_fake_metric.flatten(), generator_outputs["one_labels"][0].float()
        )
    ...
    def calculate_discriminator_loss(self, generator_outputs):
        ...
        if pesq_score is not None:
            ...
            discrim_loss_metric = F.mse_loss(
                predict_max_metric.flatten(), generator_outputs["one_labels"][0]
    ...
    def train(self):
        ...
        g_loss = []
        d_loss = []
        model_dir = args.save_model_dir + '_' + str(time.time())
        ...
        for epoch in range(args.epochs):
            ...
            path = os.path.join(
                model_dir,
                "CMGAN_epoch_" + str(epoch) + "_" + str(gen_loss)[:5] + ".pth",
            )
            if self.gpu_id == 0:
        ...
        plt.ylabel('Loss')
        plt.legend()
        plt.savefig(str(model_dir) + os.path.sep + f'GAN_Loss_{time.time()}.png')
        #eval_epochs(g_loss,model_dir,min_epochs=1)
